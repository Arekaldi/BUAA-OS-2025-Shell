...
void page_init(void) {
	...
	for(u_long now = 0; now <= PADDR(freemem); ++now) {
		pa2page(now)->pp_ref = 1;
	}

	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */

	for(u_long now_page_num = PPN(PADDR(freemem)); now_page_num < npage; ++now_page_num) {
		LIST_INSERT_HEAD(&page_free_list, pages + now_page_num, pp_link);
		(pages + now_page_num)->pp_ref = 0;
	}
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	u_long pa = ((PDX(*pgdir_entryp) << 10) | PTX(*pgdir_entryp)) << 12;
...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	int res = pgdir_walk(pgdir, va, 1, &pte);
	if(res == -E_NO_MEM)
		return res;
