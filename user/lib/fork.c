...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if((perm & PTE_COW) == 0)
		user_panic("cow_entry perm have not PTE_COW");
	...
	u_long addr = va / PAGE_SIZE * PAGE_SIZE;

	memcpy((void *)UCOW, (void *)addr, PAGE_SIZE);
 ...
 *     kernel 'envid2env' converts '0' to 'env').
...
static void duppage(u_int envid, u_int vpn) {
	...
	if((perm & PTE_D) == 0 || perm & PTE_COW || perm & PTE_LIBRARY) {
		r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
		if(r < 0)
			user_panic("duppage returned %d", r);
	}
	else if(perm & PTE_D && (perm & PTE_COW) == 0 && (perm & PTE_LIBRARY) == 0) {
...
int fork(void) {
	...
	u_long pageDirNum = PDX(USTACKTOP);

	for(i = 0; i <= pageDirNum; ++i) {
		if(vpd[i] & PTE_V) {
			for(int j = 0; j < PAGE_SIZE / sizeof(Pte); ++j) {
				u_long va = (i * (PAGE_SIZE / sizeof(Pte)) + j) * PAGE_SIZE;
				if(va >= USTACKTOP)
					break;
				if(vpt[VPN(va)] & PTE_V)
